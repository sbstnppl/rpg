# Scene-First Architecture - Schema Definitions

This document contains all Pydantic schemas for the Scene-First Architecture. Copy these into `src/world/schemas.py` as a starting point.
...
class NPCSpec(BaseModel):
    ...
    relationship_to_player: str | None = Field(
        default=None,
        description="e.g., 'school friend', 'neighbor', 'stranger'"
    )
...
class NPCPlacement(BaseModel):
    ...
    entity_key: str | None = Field(
        default=None,
        description="Existing NPC key, or None if new NPC"
    )
    new_npc: NPCSpec | None = Field(
        default=None,
        description="Spec for new NPC if entity_key is None"
    )
...
class NewElement(BaseModel):
    ...
    constraints_checked: list[str] = Field(
        ...
        description="Which constraints were verified"
    )
...
class FurnitureSpec(BaseModel):
    ...
    container_state: str | None = Field(
        default=None,
        description="If container: 'closed', 'open', 'locked'"
    )
...
class Atmosphere(BaseModel):
    ...
    weather_effects: str | None = Field(
        default=None,
        description="'rain pattering on window'"
    )
...
# World Mechanics outputs WorldUpdate
world_update = WorldUpdate(
    npcs_at_location=[
        NPCPlacement(
            entity_key="marcus_001",
            presence_reason=PresenceReason.STORY,
            presence_justification="Marcus is the player's school friend visiting",
            activity="sitting on the bed reading",
            mood="relaxed",
...
# Scene Builder outputs SceneManifest
scene = SceneManifest(
    ...
    furniture=[...],
    items=[...],
    npcs=[...],  # Populated from world_update
    atmosphere=Atmosphere(
        lighting="soft morning light",
        lighting_source="window",
        sounds=["birds outside"],
        smells=["fresh linen"],
...
# Persister builds NarratorManifest
manifest = NarratorManifest(
    ...
    entities={
        "marcus_001": EntityRef(
            ...
            display_name="Marcus",
            entity_type="npc",
            short_description="your school friend",
            pronouns="he/him",
            position="on the bed",
        ),
        "bed_001": EntityRef(
            ...
            short_description="a sturdy wooden bed with a feather mattress",
...
# Narrator uses manifest.get_reference_guide() in prompt
```
